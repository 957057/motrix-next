...
const props = withDefaults(defineProps<{ values: number[]; capacity?: number; height?: number }>(), {
  capacity: 90,
  height: 28,
})
...
<template>
  <div ref="host" class="sparkline" :style="{ height: `${height}px` }" aria-hidden="true">
    <svg v-if="geometry.line" :width="width" :height="height" :viewBox="`0 0 ${width} ${height}`">
      ...
      <path :d="geometry.line" fill="none" stroke="var(--rb-accent)" stroke-width="1.5" stroke-linejoin="round" />
    </svg>
  </div>
</template>
