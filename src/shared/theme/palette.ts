...
export function buildThemeTokens({ seed, neutral, dark }: PaletteInput): ThemeTokens {
  ...
  const a = (l: number, boost = 1, alpha?: number): string => hex({ l, c: accentChroma(chroma, l) * boost, h, alpha })
  const s = (hue: number, l: number, c: number, alpha?: number): string => hex({ l, c, h: hue, alpha })
